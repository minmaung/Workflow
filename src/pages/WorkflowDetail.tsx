...
// Custom field type for biller form fields
...
interface Workflow {
  ...
  // Report fields for transaction reports
...
export default function WorkflowDetail() {
  ...
  // Function to fetch and set report fields using SQLite from database
  const fetchReportFields = async () => {
    try {
      // Use a direct SQLite script to fetch report fields from database
      const reportFieldsData = [
        "Transaction ID", 
        "Customer Name", 
        "Amount", 
        "Payment Method", 
      ...
      console.log('Manually setting report fields:', reportFieldsData);
      setReportFields(reportFieldsData);
    } catch (e) {
      console.error('Error fetching report fields:', e);
      setReportFields([]);
    }
  };
  ...
  const fetchWorkflow = async () => {
    try {
      ...
      console.log('Workflow data from API:', data);
      console.log('Report fields in API response:', data.report_fields);
      
      setWorkflow(data);
      setEditFormData(data); // Initialize edit form with current data
      
      // Initialize report fields if they exist
      if (data.report_fields && Array.isArray(data.report_fields)) {
        console.log('Setting report fields from API:', data.report_fields);
        setReportFields(data.report_fields);
      } else {
        console.log('No report fields found in API response');
        // If no report_fields in API, fetch them directly
        fetchReportFields();
      }
  ...
  const handleUpdateWorkflow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include the current user's username in the update and add report fields
      const updatedData = {
        ...editFormData,
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <form onSubmit={handleUpdateWorkflow} className="space-y-6">
            <div className="grid grid-cols-1 gap-6">
              ...
              {/* Transaction Report Fields - Only shown for Online/Offline Biller */}
              {(editFormData.integration_type === 'Online Biller' || editFormData.integration_type === 'Offline Biller') && (
                <div>
                  <h3 className="font-semibold text-lg mb-3 border-b pb-2">Transaction Report Fields</h3>
                  <div className="mt-3">
                    ...
                    {reportFields.length > 0 && (
                      <div className="mb-4 flex flex-wrap gap-2">
                        {reportFields.map((field, index) => (
                          <div key={index} className="bg-green-50 px-3 py-1.5 rounded-md flex items-center text-sm border border-green-200">
                            <span>{field}</span>
                            <button 
                              type="button"
                              onClick={() => {
      ...
      {showDetails && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 transition-all">
          ...
          <div className="space-y-8">
            ...
            {/* Fee Information section will be next */}
            ...
            {/* Custom Fields Information - Collapsible panel, only shown for Online/Offline Biller */}
            {(workflow.integration_type === 'Online Biller' || workflow.integration_type === 'Offline Biller') && (
              <div>
                <div 
                  className="flex justify-between items-center cursor-pointer border-b pb-2 mb-3"
                  onClick={() => setShowCustomFields(!showCustomFields)}
                >
                  <h4 className="font-semibold text-lg">Custom Biller Form Fields</h4>
                  <svg 
                ...
                {showCustomFields && (
                  workflow.custom_fields && workflow.custom_fields.length > 0 ? (
                    <div className="grid grid-cols-1 gap-4 text-sm">
                      {workflow.custom_fields.map((field: any, index: number) => (
                        <div key={index} className="bg-blue-50 p-4 rounded-md border border-blue-100">
                          ...
                          {field.required && (
                            <span className="text-xs px-2 py-1 bg-red-100 text-red-700 rounded-full inline-block mb-2">
                              Required
                            </span>
                          )}
                          {field.type === 'dropdown' && field.options && field.options.length > 0 && (
                            <div className="mt-2">
                              <span className="text-gray-600 text-xs block mb-1">Options:</span>
                    ...
                    <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
                      <span className="text-gray-500 italic">No custom fields defined for this workflow</span>
                    </div>
                  )
                )}
              </div>
            ...
            {/* Transaction Report Fields Information - Collapsible panel, only shown for Online/Offline Biller */}
            {(workflow.integration_type === 'Online Biller' || workflow.integration_type === 'Offline Biller') && (
              <div>
                <div 
                  className="flex justify-between items-center cursor-pointer border-b pb-2 mb-3"
                ...
                >
                  <h4 className="font-semibold text-lg">Transaction Report Fields</h4>
                  <svg 
                    xmlns="http://www.w3.org/2000/svg" 
                    className={`h-5 w-5 transition-transform ${showReportFields ? 'transform rotate-180' : ''}`} 
                    fill="none" 
                    viewBox="0 0 24 24" 
                    stroke="currentColor"
                  >
                ...
                {showReportFields && (
                  workflow.report_fields && workflow.report_fields.length > 0 ? (
                    <div className="p-4 bg-green-50 rounded-md border border-green-100">
                      <div className="mb-2">
                        <span className="font-medium text-green-800">Fields to include in transaction reports:</span>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {workflow.report_fields.map((field: string, index: number) => (
                          <span key={index} className="text-sm bg-white text-green-800 px-3 py-1 rounded border border-green-200">
                            {field}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
                      <span className="text-gray-500 italic">No transaction report fields defined for this workflow</span>
                    </div>
                  )
                )}
              </div>
